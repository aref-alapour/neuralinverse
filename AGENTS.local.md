...
4. **Live-patch gate:** every source change ships in the SAME task with its
   ...
   "Applied" means **apply green AND `--verify` green** — the script's own
   acceptance test: `python tools/live-patch.py --verify` must report every
   patch OK and exit 0 (it checks `new` FIRST — insertion patches keep their
   `old` visible after applying, so "old is still present" is NOT "not
   applied"). A bare apply run proves nothing: the old script exited 0 even
   while skipping every patch, which is how a whole pipeline once went live
   with zero effect. A commit without its live-patch port + green verify is
   NOT done. After ANY app update, `--status` tells whether the `.orig`
   baselines went stale — `--rebaseline` re-adopts them; never let an old
   `.orig` be written back over a newer install.
...
- After ANY app update: files are replaced → tell the owner to run the
  desktop shortcut **"Repatch NeuralInverse.bat"**, then run
  `python tools/live-patch.py --status` — if baselines went STALE (app
  updated), `--rebaseline` first, then apply and `--verify` every patch; if a
  pattern went missing, update the script for the new bundle BEFORE any new
  work.
...
3. Implement on source; run `node tools/verify.mjs` — it must be GREEN
   (type-check slice with the repo's strict options + standalone tests +
   live-patch sandbox selftest, honest NOT-CHECKED list; task Q5) before
   committing.
4. Port to `tools/live-patch.py` (minified patterns), run elevated, then
   `--verify` green (gate 4), ask the owner to restart + test.
