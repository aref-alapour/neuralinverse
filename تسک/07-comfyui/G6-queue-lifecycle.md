...
- **اولویت:** P1 (Wave 4) | **برآورد:** M | **وضعیت:** 🔴 | **وابستگی:** G3

> **اصلاح مارکر ۲۰۲۶-۰۹-۰۸: از 🟡 به 🔴.** مارکر قبلی («زیرساخت صف ما جور دیگر
> هست») به صف *background agent* و run-history *workflow* اشاره داشت — نه به صف
> گراف که موضوع این تسک است. طبق قاعده‌ی وضعیت در [Q8](../05-quality/08-open-debt-register.md)
> بخش ۰، 🟡 یعنی «کد این تسک نوشته و کامیت شده»؛ برای G6 صفر از چهار معیار
> پذیرش کد دارد. ضمناً همان صف background agent هم ثبت نشده است (نک. A1).
>
> پایه‌ی قابل‌استفاده‌ی مجدد هنگام شروع: `workflowOrchestrator` (لایه‌بندی
> توپولوژیک + cancellation) و run-history با سقف ۵۰ در `workflowAgentService.ts:112`.
