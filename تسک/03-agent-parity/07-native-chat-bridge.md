...
- **اولویت:** **P0 — پرلوریج‌ترین تسک این بک‌لاگ** | **برآورد:** M | **وضعیت:** 🟡 بندهای الف، ب، ج، د بسته (`f332502e5dc`، `4c1e1c16b56`)؛ ه + checkpoint باز
...
### ج) ~~Ledger به این مسیر وصل نیست~~ ✅ بسته شد (`4c1e1c16b56`)

حلقه حالا نوبت کاربر، هر نوبت assistant (با متادیتای model و usage) و هر نتیجه‌ی
ابزار را append می‌کند — با **یک thread ledger به ازای هر session چت**، تا session
از سرگرفته‌شده بایگانی‌اش را نگه دارد. فلگ `contextLedgerEnabled` رعایت می‌شود و
نوشتن best-effort است: خطای ledger هرگز نباید نوبت چت را بشکند (همان قرارداد سایدبار).

### د) ~~رویداد usage نمی‌فرستد~~ ✅ بسته شد (`4c1e1c16b56`)

حلقه بعد از هر فراخوانی مدل، `{ kind: 'usage' }` را با `actualModelId` می‌فرستد تا
gauge بتواند پنجره‌ی context مدل BYOLLM ما را resolve کند. **به ازای هر iteration**
گزارش می‌شود نه یک‌بار در پایان: `promptTokens` یعنی «الان پنجره چقدر پر است» و یک
نوبت agent چندین بار مدل را صدا می‌زند؛ هسته خودش `completionTokens` را جمع می‌بندد.

> ⚠️ این بند فقط روی desktop عدد واقعی می‌دهد چون به
> [Q12](../05-quality/12-desktop-usage-parity.md) (`213acb14709`) وابسته است.
