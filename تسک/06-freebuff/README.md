# پورت‌های freebuff (Codebuff snapshot) — کاتالوگ راستی‌آزمایی‌شده

> منبع: `C:\Users\jobal\dev\freebuff` — snapshot عمومی Codebuff (بازبرند freebuff)،
> ~۳۴k خط TS، Bun monorepo. **لایسنس Apache 2.0 + فایل `NOTICE`** → پورت فقط با
> کپی/تطبیق + حفظ attribution (هدر در فایل + ثبت در `ThirdPartyNotices.txt` ریشه).
> **هرگز dependency اضافه نشود** — منطق پورت می‌شود، نه import ها (Bun/Zod/AI-SDK
> با layering ما سازگار نیست).
>
...
## قانون Attribution (الزام قانونی)

هر فایلی که از freebuff پورت/تطبیق می‌شود:
1. هدر کامنت: `// Adapted from Codebuff/Freebuff (Apache-2.0, Copyright (c) Codebuff) — source: <path>`
2. یک ردیف در `ThirdPartyNotices.txt` ریشه‌ی repo
3. بازنویسی به سبک TS ما (نه کپی بی‌وجه) — هم لایسنس هم کیفیت
