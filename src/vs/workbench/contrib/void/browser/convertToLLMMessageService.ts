...
import { formatWorkspaceRuleFiles, WORKSPACE_RULE_FILENAMES, IWorkspaceRuleFile } from '../common/workspaceRuleFiles.js';
	...
	for (let i = 0; i < messages.length; i += 1) {
		...
		if (currMsg.role === 'tool') {
			...
			if (prevAssistantMsg?.role === 'assistant') {
				...
				const assistantBlocks = prevAssistantMsg.content as { type: 'tool_use'; name: string; input: Record<string, unknown>; id: string }[];
				assistantBlocks.push({ type: 'tool_use', id: currMsg.id, name: currMsg.name, input: currMsg.rawParams });
			}
	...
	for (let i = 0; i < newMessages.length; i++) {
		...
		if (msg?.role === 'user' && Array.isArray(msg.content) && msg.content[0]?.type === 'tool_result') {
			...
			while (i + 1 < newMessages.length) {
				...
				if (next?.role === 'user' && Array.isArray(next.content) && next.content[0]?.type === 'tool_result') {
					toolResults.push(...next.content);
					i++;
				} else { break; }
			}
			merged.push({ role: 'user', content: toolResults });
		} else {
			merged.push(msg);
		}
	}
...
const prepareMessages_XML_tools = (messages: SimpleLLMMessage[], supportsAnthropicReasoning: boolean): AnthropicOrOpenAILLMMessage[] => {
	...
	for (let i = 0; i < messages.length; i += 1) {
		...
		// add user or tool to the previous user message
			...
			if (c.role === 'tool') { contentToAdd = `<${c.name}_result>\n${contentToAdd}\n</${c.name}_result>`; }
...
const prepareOpenAIOrAnthropicMessages = ({
	...
	if (aiInstructions) { sysMsgParts.push(`GUIDELINES (from the user's AGENTS.md / CLAUDE.md / .neuralinverserules files and their settings):\n${aiInstructions}`); }
...
class ConvertToLLMMessageService extends Disposable implements IConvertToLLMMessageService {
	...
	// Read workspace rule files (AGENTS.md / CLAUDE.md / .neuralinverserules —
	// task E1) from every workspace folder. File names come from the shared
	// workspaceRuleFiles module so the native chat's collector and this reader
	// stay in sync.
	private _getWorkspaceRuleFileContents(): string {
		try {
			...
			for (const folder of workspaceFolders) {
				for (const fileName of WORKSPACE_RULE_FILENAMES) {
					...
					try {
						...
						if (!model) { continue; }
						ruleFiles.push({ fileName, content: model.getValue(EndOfLinePreference.LF) });
					} catch (e) {
	...
	private _getCombinedAIInstructions(agentContext: string): string {
		...
		if (workspaceRuleFileContent) { ans.push(workspaceRuleFileContent); }
